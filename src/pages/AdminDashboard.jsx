...
export default function AdminDashboard() {
    ...
    async function handleToggleHidden(table, id, currentStatus, fetcher) {
    ...
    async function handleSaveExperience(e) {
        ...
        const endDate = rawEndDate ? parseFlexibleDate(rawEndDate) : null;
        ...
        if (editingExperience) {
            const res = await supabase.from("experience").update(experienceData).eq("id", editingExperience.id);
            error = res.error;
        } else {
            const res = await supabase.from("experience").insert([experienceData]);
            error = res.error;
        }
    ...
    return (
        <div className="min-h-screen pt-32 px-6 md:px-12 max-w-7xl mx-auto pb-24 relative overflow-hidden">
            <FlickeringGrid
            ...
            <div className="relative z-10">
                ...
                {activeTab === "projects" && (
                    <div>
                        ...
                        {isFormOpen ? (
                            <div className="mb-12 border border-border p-6 bg-background/50 backdrop-blur-sm">
                            ...
                            <div className="space-y-4">
                                {projects.map((project) => (
                                    <div key={project.id} className="border border-border p-4 flex justify-between items-center bg-background/50 backdrop-blur-sm">
                                        <div>
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => handleToggleHidden("projects", project.id, project.is_hidden, fetchProjects)}
                                                className="text-xs uppercase font-bold hover:opacity-50"
                                                title={project.is_hidden ? "Show" : "Hide"}
                                            >
                                                {project.is_hidden ? <EyeOff size={16} /> : <Eye size={16} />}
                                            </button>
                                            <button
                ...
                {activeTab === "experience" && (
                    <div>
                        ...
                        {isFormOpen ? (
                            ...
                            <div className="space-y-4">
                                {experience.map((exp) => (
                                    <div key={exp.id} className="border border-border p-4 flex justify-between items-center bg-background/50 backdrop-blur-sm">
                                        <div>
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => handleToggleHidden("experience", exp.id, exp.is_hidden, fetchExperience)}
                                                className="text-xs uppercase font-bold hover:opacity-50"
                                                title={exp.is_hidden ? "Show" : "Hide"}
                                            >
                                                {exp.is_hidden ? <EyeOff size={16} /> : <Eye size={16} />}
                                            </button>
                                            <button
                    ...
                    <div>
                        ...
                        {isFormOpen ? (
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                {skills.map((skill) => (
                                    <div key={skill.id} className="border border-border p-4 flex justify-between items-center bg-background/50 backdrop-blur-sm">
                                        <div className="min-w-0 flex-1 mr-4">
                                            <div className="font-bold break-words">{skill.name}</div>
                                            <div className="text-xs text-muted-foreground break-words">{skill.category}</div>
                                        </div>
                                        <div className="flex gap-2 shrink-0">
                                            <button
                                                onClick={() => handleToggleHidden("skills", skill.id, skill.is_hidden, fetchSkills)}
                                                className="text-xs uppercase font-bold hover:opacity-50"
                                                title={skill.is_hidden ? "Show" : "Hide"}
                                            >
                                                {skill.is_hidden ? <EyeOff size={16} /> : <Eye size={16} />}
                                            </button>
                                            <button
                        ...
                        {isFormOpen ? (
                            ...
                            <div className="space-y-4">
                                {certificates.map((cert) => (
                                    <div key={cert.id} className="border border-border p-4 flex justify-between items-center bg-background/50 backdrop-blur-sm">
                                        <div>
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                onClick={() => handleToggleHidden("certificates", cert.id, cert.is_hidden, fetchCertificates)}
                                                className="text-xs uppercase font-bold hover:opacity-50"
                                                title={cert.is_hidden ? "Show" : "Hide"}
                                            >
                                                {cert.is_hidden ? <EyeOff size={16} /> : <Eye size={16} />}
                                            </button>
                                            <button
                ...
                {activeTab === "profile" && (
                    <div>
                        ...
                        <form onSubmit={handleSaveProfile} className="space-y-6">
                            ...
                            <div className="border border-border p-6 bg-background/50 backdrop-blur-sm">
                                <div className="grid grid-cols-2 gap-4">
                                    <div>
